//
// Created by yoj49 on 12/02/2024.
//
#include "Encoder.h"
...
#define RAW_H264_FILE_PATH  "/sdcard/DCIM/Camera/%02d_%02d_%04d_%02d:%02d:%02d.h264" //"/storage/19EA-3759/seesee_smart_module/%04d_%02d_%02d_%02d:%02d:%02d.h264"
            ...
            */
            ...
            if (info.flags == 1) {
                if (record == true) {
                    ...
                    file_ok = initFile(RAW_H264_FILE_PATH, JSON_H264_FILE_PATH, tm, now);
                    if (file_ok) {
                        jsoninit_ok = json_init();
                    }
...
void Encoder::InitCodec(int height , int width, int framerate ,int bitrate)
{
    ...
    */
    ...
    mStatus = AMediaCodec_configure(mMediaCodec, mMediaFormat, nullptr/* surface */, nullptr /* crypto*/,
...
void Encoder::Encode (unsigned char *YUV_NV12, int planeSize)
{
    ...
    */
    ...
    //if (inputBufId > 0) {
        ...
        //bufferPointer = AMediaCodec_getInputBuffer(mMediaCodec, static_cast<size_t>(inputBufId), &planeSize);

        if (bufferPointer) {

            //memcpy(bufferPointer,"test",4); => BufferPointer est bien une zone de la mémoire ou on peut écrire
...
/*bool Encoder::json_init()
{

    mroot_value = cg_json_value_init_object();
    if (mroot_value == NULL) {
        printf("rootValue cannot init \n");

    }
    mroot_object = cg_json_value_get_object(mroot_value);

    cg_json_object_set_string(mroot_object, "version", "0.1.0");

    cg_json_object_set_string(mroot_object, "date", mjsonfilename);

    marray_value = cg_json_value_init_array();
    if (marray_value == NULL) {
        printf("arrayValue cannot init \n");
    }

    marray_object = cg_json_value_get_array(marray_value);

    return true;
}*/

/*bool Encoder::json_addvalue(int tab)
{
    cg_json_array_append_number(marray_object, tab);

    cg_json_object_set_value(mroot_object, "pframes", marray_value);
...
/*bool Encoder::json_close()
{

    cg_json_serialize_to_file(mroot_value, mjsonfilename);
    //printf(" serialize_to_file ok \n");

    cg_json_value_free(mroot_value);
    return true;
}*/

bool Encoder::initFile(const char* H264_FILE_PATH, const char* JSON_FILE_PATH,
                       struct tm* timeInfos, time_t timestamp)
{
    ...
    int retValPahtJson = sprintf(mjsonfilename, JSON_FILE_PATH, timeInfos->tm_mday, timeInfos->tm_mon+1, timeInfos->tm_year+1900,
                                 timeInfos->tm_hour, timeInfos->tm_min, timeInfos->tm_sec);
...
void Encoder::setSocketClientH264(SocketClient *m_ClientH264) {
    this->mClienth264 = m_ClientH264;
}